...
|---|---|---|---|---|---|---|---|---|
| A10 | Indicate User Type | Indicate which type of user they are | User | None | User selects their user type from options provided | User type is stored in the system | User type selection is confirmed | Ensure clear options are available for user types |
| A11 | Enter Email | Enter their email | User | User indicated "Candidate" | User enters their email in the provided field | Email is stored in the system | Email entry is confirmed | Validate email format |
| A12 | Enter Password | Enter their password | User | User indicated "Candidate" | User enters their password in the provided field | Password is stored in the system | Password entry is confirmed | Ensure password meets security criteria |
| A13 | Enter Birthday | Enter their birthday | User | User indicated "Candidate" | User enters their birthday in the provided field | Birthday is stored in the system | Birthday entry is confirmed | Validate date format |
| A14 | Enter Phone Number | Enter their phone number | User | User indicated "Candidate" | User enters their phone number in the provided field | Phone number is stored in the system | Phone number entry is confirmed | Validate phone number format |
| A15 | Click Sign Up | Click on sign up | User | User has entered all required information | User clicks the sign-up button | User data is submitted to the system | User data submission is confirmed | Ensure all fields are filled |
| A16 | Receive Confirmation Email | Candidate receives an email with confirmation link | System | User has clicked sign up | System sends a confirmation email to the user's email address | Confirmation email is sent | User receives the confirmation email | Ensure email delivery |
| A17 | Confirm Email Link | Link reopens the app to the link confirmation page | User | User has received the confirmation email | User clicks the confirmation link in the email | App opens to the link confirmation page | User is redirected to the confirmation page | Ensure link opens correctly |
| A18 | Notification Permission | Asked if can get notifications | System | User is on the confirmation page | System prompts user to allow notifications | User's notification preference is stored | User accepts or declines notifications |  |
| A19 | Onboarding Page | Candidate is brought to the onboarding page | User | User has completed email confirmation | User is redirected to the onboarding page | User begins onboarding process | Onboarding page is displayed |  |
...
| A215 | View Job Offer Details | The Job offer expands and they see the detail of soft skills matching | System | User is viewing a job offer page | System displays detailed soft skills matching information | Candidate views the detailed soft skills matching | Detailed information is visible and comprehensible | Ensure details are accurate and relevant |
| A216 | View Company Profile and Rate | Candidate is shown that they can click on the company's profile for further information and that they can even rate it if they want to express their preferences | System | User is viewing a job offer page | System displays options to view company profile and rate the company | Candidate clicks to view profile and/or rates the company | Company profile is viewed, rating is submitted | Ensure profile and rating features are accessible and functional |
| A217 | Return to Profile | Candidate is brought back to their profile saying "Thank you for following this introduction, now it is time for you to tell us more about you!" | System | User has completed the job offer exploration | System redirects candidate back to their profile with a thank you message | Candidate is returned to their profile page | Candidate is ready to provide more personal information | Ensure the transition back to the profile page is smooth and the message is encouraging |
...
| Use Case Number | Name | Description | Actor(s) | Pre-Conditions | Flow of Events | Post-Conditions | Exit Criteria | Notes and Issues |
|---|---|---|---|---|---|---|---|---|
| A300 | Pick Languages | Candidate is asked to pick the languages they are familiar with | User | User is on the profile creation page | User selects languages from a provided list | Selected languages are stored in the system | User confirms language selection | Ensure language list is comprehensive |
| A301 | Pick Location | Candidate picks their location with the map | User | User is on the profile creation page | User interacts with the map to set their location | Location is stored in the system | User confirms location selection | Ensure map is interactive and accurate |
| A30110 | Provide Location | Candidate is asked to provide their location for easier input of address | User | User is on the location input step | User provides their location | Location input field is populated | User moves to the next step | Ensure location input is user-friendly |
| A30111 | Type Address | Candidate types their address | User | User is on the address input step | User types their address in the input field | Address is stored in the system | User moves to the next step | Ensure address field validates input correctly |
| A30112 | Validate Address | Candidate selects the address from a dropdown or moves the pinpoint to their exact location and presses enter to validate | User | User has typed their address | User selects address from dropdown or adjusts pinpoint on the map | Address is validated and stored in the system | User confirms address selection | Ensure dropdown and map interaction are accurate |
| A30113 | Select Range | Candidate uses a slider to select the range they are willing to go to | User | User has validated their address | User adjusts the range slider to set their willing travel distance | Range is stored in the system | User confirms range selection | Ensure slider is easy to use and accurate |
| A30114 | Validate Location Preferences | Candidate validates their localization preferences | User | User has set their location and range | User reviews and confirms their location preferences | Preferences are stored in the system | User completes location setup | Ensure user can easily review and confirm preferences |
| A302 | Pick Pseudonym | Candidate picks their pseudonym | User | User is on the profile creation page | User selects or generates a pseudonym | Pseudonym is stored in the system | User confirms pseudonym selection | Ensure pseudonym options are appropriate and neutral |
| A3020 | Generate Pseudonym | Candidate can generate one with AI based on their location and language | User | User is on the pseudonym selection step | User generates pseudonym using AI | Generated pseudonym is stored in the system | User confirms pseudonym selection | Ensure AI generates neutral and appropriate names |
| A3021 | Type Pseudonym | Or they can type one, based on their preference | User | User is on the pseudonym selection step | User types their preferred pseudonym | Typed pseudonym is stored in the system | User confirms pseudonym selection | Ensure pseudonym input is validated |
| A3022 | Attribute Animal | They are attributed an animal from the google anonymous animals list | System | User has selected or generated a pseudonym | System assigns an anonymous animal to the user | Animal is stored in the system | User sees their animal assignment | Ensure animals are assigned fairly and are neutral |
| A303 | Upload CV | Candidate presses the "Upload a CV" Button | User | User is on the profile creation page | User presses the "Upload a CV" button | CV upload process starts | CV is uploaded and stored | Ensure upload process is smooth and supports various formats (pdf and docx at the very least) |
| A304 | Upload Video Story | Candidate presses the "Upload a video story" button | User | User is on the profile creation page | User presses the "Upload a video story" button | Video upload process starts | Video is uploaded and stored | Ensure upload process is smooth and supports basic formats (mp4 for now) |
| A305 | Write Bio | Candidate presses the "Write a bio" button | User | User is on the profile creation page | User presses the "Write a bio" button | Bio writing interface opens | Bio is written and stored | Ensure bio interface is user-friendly |
| A306 | Upload Portfolio | Candidate presses the "Upload a portfolio" Button | User | User is on the profile creation page | User presses the "Upload a portfolio" button | Portfolio upload process starts | Portfolio is uploaded and stored | Ensure upload process is smooth and supports various formats |
| A307 | Favorite Books | Candidate presses the "Favorite Books" button | User | User is on the profile creation page | User presses the "Favorite Books" button | Favorite books input interface opens | Favorite books are added and stored | Ensure input interface is user-friendly |
| A308 | Favorite Movies | Candidate presses the "Favorite Movies" Button | User | User is on the profile creation page | User presses the "Favorite Movies" button | Favorite movies input interface opens | Favorite movies are added and stored | Ensure input interface is user-friendly |
| A309 | Psychometric Assessment | Candidate presses the "Psychometric assessment" button | User | User is on the profile creation page | User presses the "Psychometric assessment" button | Assessment process starts | Assessment results are stored | Ensure assessment process is smooth and accurate |
| A3010 | Add Certification | Candidate presses the "add a certification" button | User | User is on the profile creation page | User presses the "add a certification" button | Certification addition interface opens | Certification is added and stored | Ensure input interface is user-friendly |
| A3011 | Play Game Assessments | Candidate presses the "play game assessments" button | User | User is on the profile creation page | User presses the "play game assessments" button | Game assessment process starts | Assessment results are stored | Ensure game assessments are engaging and accurate |
| A3012 | Watch Professional Resources | Candidate presses the "Watch professional resources" button | User | User is on the profile creation page | User presses the "Watch professional resources" button | User is redirected to resources tab | Resources are viewed | Ensure resources are informative and valuable |
| A3013 | View Score Information | Candidate clicks on their score to have more information on how scores are calculated | User | User is on the profile creation page | User clicks on their score | Score calculation information is displayed | User understands score calculation | Ensure information is clear and detailed |
...
| Use Case Number | Name | Description | Actor(s) | Pre-Conditions | Flow of Events | Post-Conditions | Exit Criteria | Notes and Issues |
|---|---|---|---|---|---|---|---|---|
| A310 | Click Profile Button | Click on the profile button | User | User is logged into the app | User clicks on the profile button | Profile page is opened | User's profile page is displayed | Ensure profile button is easily accessible |
| A311 | View Video Story | User views their own video story | User | User is on their profile page | User clicks on the video story section | Video story starts playing | Video story is viewed | Ensure video plays smoothly and is of good quality (720p min) |
| A312 | Share Profile | User clicks on "share my profile" | User | User is on their profile page | User clicks on the "share my profile" button | Sharing options are displayed | Profile sharing process is initiated | Ensure sharing options are clear and functional |
...
| Use Case Number | Name | Description | Actor(s) | Pre-Conditions | Flow of Events | Post-Conditions | Exit Criteria | Notes and Issues |
|---|---|---|---|---|---|---|---|---|
| A40 | Start Matching | Candidate clicks on the target icon to start matching and is presented with a match | User | User is logged into the app and on the main page | User clicks on the target icon | Matching page is opened with a presented match | User views the first job offer | Ensure target icon is easily clickable |
| A41 | Swipe Offer Right | Candidate swipes an offer right | User | User is on the matching page viewing a job offer | User swipes the job offer card to the right | Offer is accepted and saved | User proceeds to the next job offer | Ensure swipe right interaction is smooth and responsive |
| A42 | Swipe Offer Left | Candidate swipes an offer left | User | User is on the matching page viewing a job offer | User swipes the job offer card to the left | Offer is rejected | User proceeds to the next job offer | Ensure swipe left interaction is smooth and responsive |
| A43 | Get Offer Details | Candidate clicks on an offer to get more details | User | User is on the matching page viewing a job offer | User clicks on the job offer card | Detailed job offer page is opened | User views detailed information about the job offer | Ensure transition to detailed view is smooth and detailed information is accurate |
...
| Use Case Number | Name | Description | Actor(s) | Pre-Conditions | Flow of Events | Post-Conditions | Exit Criteria | Notes and Issues |
|---|---|---|---|---|---|---|---|---|
| A50 | Sort Resources with Filters | Candidate sorts the resources with filters to see what they want to see | User | User is on the professional resources page | User applies filters to sort resources | Resources are filtered based on selected criteria | User views sorted resources | Ensure filters are intuitive and cover key criteria |
| A51 | Search with Keyword | Candidate searches with a keyword | User | User is on the professional resources page | User enters a keyword in the search bar | Resources matching the keyword are displayed | User views search results | Ensure search functionality is accurate and responsive |
| A52 | Get Recommendations | Candidate can also let the app recommend based on their previous matches to either capitalize on their existing soft skills or fill up their gaps in lacking ones | System | User is on the professional resources page | System provides recommendations based on previous matches | Recommended resources are displayed to the user | User views recommended resources | Ensure recommendations are relevant and personalized |
| A53 | Watch Resource Video | Candidate can click on a resource (for now only videos) and can watch them for additional points. The video experience is the same as any video experience except it doesn't allow to fast forward in the video to prevent cheating | User | User is on the professional resources page and has selected a video | User clicks on a video resource | Video player opens and plays the video without fast-forwarding | User watches the video and earns additional points | Ensure video playback is smooth and fast-forwarding is disabled |
...
| Use Case Number | Name | Description | Actor(s) | Pre-Conditions | Flow of Events | Post-Conditions | Exit Criteria | Notes and Issues |
|---|---|---|---|---|---|---|---|---|
| A60 | Click on Matched Job Offer | Candidate clicks on a recently matched job offer in their Chat area | User | User is in the Chat and Interviews section with matched job offers | User clicks on a matched job offer | Chat with the company opens | User views conversation with the company | Ensure matched job offers are clearly displayed |
| A61 | Respond to First Message | Candidate responds to the company's first message | User | User has received a message from the company | User types and sends a response message | Message is sent to the company | User's response is logged in the chat | Ensure messaging interface is intuitive and reliable |
| A62 | Answer AI Interview | Candidate answers the AI interview when they are ready | User | User has received an AI interview prompt | User starts and completes the AI interview | Interview responses are submitted | AI interview is completed and responses are stored | Ensure AI interview process is smooth and user-friendly |
| A63 | Close Discussion | Candidate can close the discussion if they are uninterested | User | User is in a conversation with a company | User opts to close the discussion | Discussion is closed and marked as uninterested | User exits the chat | Ensure option to close discussion is clear and accessible |
| A630 | Rate Exchange | Candidate can rate the exchange | User | User has completed or closed a discussion | User rates the exchange experience | Rating is submitted and stored | Exchange is rated | Ensure rating system is easy to use |
| A631 | Report Exchange | Candidate can report the exchange | User | User has completed or closed a discussion | User reports the exchange for issues | Report is submitted and logged | Exchange is reported | Ensure reporting system is clear and responsive |
| A64 | Accept Interview Proposition | Candidate can accept a proposition from a company to move from candidate status to Interviewee, indicating that they will be interviewed | User | User has received an interview proposition | User accepts the interview proposition | Status is updated to Interviewee | Interview is scheduled and confirmed | Ensure status update is clear and confirmation is sent |
